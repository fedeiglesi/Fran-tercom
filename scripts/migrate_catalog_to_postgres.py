#!/usr/bin/env python3
"""
Script de migración del catálogo CSV a PostgreSQL (Railway).
...
def migrate_catalog():
    """Migra el catálogo CSV a PostgreSQL."""
    ...
    print(f"📊 {len(rows)} productos parseados del CSV")
    ...
    try:
        with conn.cursor() as cur:
            # Crear tabla
            print("🏗️  Creando tabla 'catalogo' si no existe...")
            cur.execute("""
            ...
            cur.execute("""
                CREATE INDEX IF NOT EXISTS idx_catalogo_marca ON catalogo(marca_moto);
            """)
            cur.execute("""
                CREATE INDEX IF NOT EXISTS idx_catalogo_modelo ON catalogo(modelo_moto);
            """)
            cur.execute("""
                CREATE INDEX IF NOT EXISTS idx_catalogo_categoria ON catalogo(categoria_final);
            """)
            cur.execute("""
                CREATE INDEX IF NOT EXISTS idx_catalogo_familia ON catalogo(familia);
            """)
            cur.execute("""
                CREATE INDEX IF NOT EXISTS idx_catalogo_descripcion_gin ON catalogo
                    USING gin(to_tsvector('spanish', COALESCE(descripcion_normalizada, '')));
            """)

            # Bulk upsert con execute_values
            print("💾 Insertando/actualizando productos...")
            execute_values(
                cur,
                """
        ...
        print(f"   💵 Con precio USD: {stats[1]}")
        print(f"   💰 Con precio ARS: {stats[2]}")
        print(f"   🏭 Marcas distintas: {stats[3]}")
        print(f"   📁 Categorías: {stats[4]}")
